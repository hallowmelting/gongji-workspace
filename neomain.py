...
while True:
    try:
            ...
            if W < 5: # 평일에만 작동
                if 9 <= H <= 17: # 작동하는 시간대 9-17
                    ...
                    def parsingsite (link):
                        ...
                        html = req.text
                        lksdjflkf = BeautifulSoup(html, 'html.parser')
                        time.sleep(5)
                    ...
                    # 기숙사 공지 읽어옴
                    url = 'https://dorm.mju.ac.kr/bbs/dorm/929/artclList.do'
                    req = requests.get(url, verify=False, headers={'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.0.0 Safari/537.36'})
                    requests.packages.urllib3.disable_warnings(requests.packages.urllib3.exceptions.InsecureRequestWarning)
                    req.encoding = 'utf-8'    # clien에서 encoding 정보를 보내주지 않아 encoding옵션을 추가
                    ...
                    for i in range(25):
                        ...
                        with open(os.path.join(BASE_DIR, 'latest_num10.txt'), 'r+') as f_read:
                            pastnum = f_read.readline()
                            if pastnum == latest_num10:
                                title_latest = nums[i].findNext('strong').text.replace('\n','').replace('\t','')
                                link_latest = 'https://www.mju.ac.kr' + nums[i].findNext('a').get('href')
                                final_message = "[기숙사 공지사항 알림]\n\n" + title_latest + "\n\n" + link_latest
                                ...
                                with open(os.path.join(BASE_DIR, 'latest_num10.txt'), 'w+') as f_write:
                                    compas_int = int(pastnum)
                                    compas_int = compas_int + 1
                                    compas_str = str(compas_int)
                                    f_write.write(compas_str)
        ...
        bot.sendMessage(chat_id, text="%s 오류 발생. 30분 대기" %e)
        bot.sendMessage(chat_id_sumin, text="%s 오류 발생. 30분 대기" %e)
